(function () {
  ...
  QUnit.test('last', function (assert) {
    ...
    assert.strictEqual(_.last([1, 2, 3]), 3, 'can perform OO-style "last()"');
  ...
  QUnit.test('flatten', function (assert) {
    ...
    assert.deepEqual(_.flatten(list, true), [1, 2, 3, [
      [
        [4]
      ]
  ...
  QUnit.test('without', function (assert) {
    ...
    assert.deepEqual(_.without(list, {
      one: 1
    }), list, 'compares objects by reference (value case)');
    assert.deepEqual(_.without(list, list[0]), [{
      two: 2
  ...
  QUnit.test('sortedIndex', function (assert) {
    ...
    assert.strictEqual(_.sortedIndex(objects, {
      x: 25
    }, iterator), 2, 'uses the result of `iterator` for order comparisons');
    assert.strictEqual(_.sortedIndex(objects, {
      x: 35
    }, 'x'), 3, 'when `iterator` is a string, uses that key for order comparisons');
  ...
  QUnit.test('intersection', function (assert) {
    ...
    assert.deepEqual(_.intersection(stooges, leaders), ['moe'], 'can perform an OO-style intersection');
    var result = (function () {
